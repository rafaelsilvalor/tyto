...
It is not hypothetical and it is not rare. Measured on win32 across 100 captures, the packaged app
failed to answer `Page.captureScreenshot` on **15 of 80** — 19% — while the dev build answered
20 of 20. Four suspects died in the same table: not the fonts wait (`document.fonts.ready` settled
in 0–2 ms on every capture, the hung ones included), not asar packing, not the GPU, not a
0.2.0 → 0.3.0 regression. Every capture that did answer answered in 39–1 075 ms.
...
7% of packaged captures still fail. What changes is that they fail as a reported failed frame, with
