...
**Several briefs are open at once, and the renderer's state is per document** (E9.11). `src/renderer/documents.ts` holds a `Workspace` — a list of documents and the id of the active one — and every painter reads the active one rather than a module-level singleton. A document carries its editor snapshot (text, undo history, cursor and scroll, opaque and `@tyto/editor`'s), its frames and artworks, its selected format and slide, its zoom and its diagnostics. One CodeMirror serves all of them: switching writes `editor.snapshot()` into the document being left and `editor.restore()`s the one being entered, which is what keeps an undo history inside its own tab — a `setValue` would throw it away. The editor panel therefore stays `fixed`: the snapshots survive, but the view that can put one back does not.
