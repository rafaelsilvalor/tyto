...
**The npm half has never completed a run, and the reason is the `packageManager` pin rather than anything in `dependabot.yml`.** Dependabot reads `pnpm@12.3.4` out of `package.json` and installs that exact version into its updater container. pnpm 12 publishes a launcher rather than the program, so the first invocation fetches `@pnpm/exe.<platform>` — and that fetch never reaches the container's proxy, the only route out of it. `pnpm -v` exits 1, every `pnpm update --lockfile-only` after it exits 1, and all eleven dependencies fail at `update_files` reporting `unknown_error` with a `null` detail. Measured in the private job log of 2026-09-12 (`job_1572596408`): **0 of the proxy's 120 requests** were for `@pnpm/exe`, the proxy logged no error of its own, and the container's own pnpm — 11.17.0, which needs no second download — answers `pnpm -v` fine. Resolution succeeds for every dependency, which is why the grouping, the lockfile and the workspace layout are ruled out: the run dies after the latest version is found and before any of the three is read (TYTO-79).

Two things follow. The updater is dark until the pin moves off pnpm 12, which is a toolchain decision and not something a broken robot gets to force; and the grouping stays whatever is decided, because one pull request a week is the point. `tools/repo-checks/src/github-config.test.ts` fails if `packageManager` leaves the major this finding was measured against, so the note is re-read rather than inherited.
