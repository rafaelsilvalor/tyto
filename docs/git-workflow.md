...
They live under `packages/raster/src/__fixtures__/reference/` as `<artwork>.<format>.png`, one file per fixture and not one per platform. That is measured, not assumed: the Windows and Linux renders of the current corpus are identical at `threshold: 0`, 0 differing pixels on both of the shape fixtures, because `DETERMINISM_ARGS` in the Playwright adapter takes the host's opinions out of Skia's software rasterizer.

`text.feed`, the glyph fixture TYTO-61 added, is the case the platform split was actually reserved for — FreeType on Linux against Skia over DirectWrite on Windows. Its reference was recorded on Windows and **it is committed unkeyed, as a question `visual.yml` answers**: 12 of 12 re-renders are byte-identical on the machine that recorded it, so the fixture is stable; whether the two rasterizers agree is what the Linux job reports. If it goes red on the first run, the fix is a per-platform key for this one fixture, not a wider tolerance — the perturbation table in `raster.visual.test.ts` says what a looser number would stop seeing.
