...
They live under `packages/raster/src/__fixtures__/reference/` and are named `<artwork>.<format>.<platform>.png`, where the platform is `process.platform`. Chromium does not rasterize the same document identically on two operating systems — the glyph rasterizer is FreeType on Linux and Skia over DirectWrite on Windows, and filter and gradient code splits the same way — so a reference is only comparable against the platform that recorded it. `pnpm test:visual` on a platform with no reference **fails**, writes the render it would have compared into `__diff__/`, and names the file to commit; `visual.yml` uploads that folder on failure, so seeding a platform is: run the job, download the artifact, commit it. Recording locally is `UPDATE_VISUAL_REFERENCE=1 pnpm --filter @tyto/raster test:visual`, and needs `pnpm exec playwright install chromium` first.

A Playwright bump is a re-record, not a tolerance question: the diff tolerates a whole-image drift of ±1 per channel and nothing wider, which is deliberate — the measurements behind both numbers are in the header of `raster.visual.test.ts`. Re-record on the platform, in its own commit, saying which browser version it moved to.
