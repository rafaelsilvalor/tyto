...
Three runners, `fail-fast: false`, four artifacts: `dmg`, `nsis`, `portable`, `AppImage`. Windows is the only one with two, because it is the only one whose installer is the whole of what it ships — an AppImage and a mounted `dmg` both give you something you can already run, and until TYTO-136 a Windows release gave you something you had to install. The `portable` is a single self-extracting `.exe` for carrying the app, not an updater artifact; `electron-builder.yml` says which of the two it is and why the macOS `zip` is still refused. All four `--publish always` into the same release, and cancelling the survivors of a failed leg would leave that release holding whichever finished first with nothing saying what was cut short. macOS builds on `macos-latest`, which is Apple Silicon, so the `dmg` is **arm64 only** — an Intel build is a second matrix entry whenever somebody needs one.
...
**Step 6 is the one nothing automates and nothing else in this document mentions.** electron-builder creates the release as a draft and stops; a tag pushed and a workflow gone green still leaves a page nobody outside this repository can see. The proof was a draft from 2026-09-14 — `desktop-v0.1.0`, a first tag pushed to watch the three runners work, which they did — and it sat unpublished for four days with its assets parked under an `untagged-…` prefix, because a draft has no tag association until somebody publishes it. **It has been deleted, tag included** (TYTO-145): it was the first row on the releases page, above the one a tester should read, and publishing it is the same click as publishing the beta.

What it measured is kept here rather than in the artefact. Its eight assets were `latest.yml`, `latest-mac.yml`, `latest-linux.yml`, the `dmg` and its blockmap, the `AppImage`, and the `nsis` installer and its blockmap — **no `portable`**, which is the independent confirmation that TYTO-136's fourth artifact has never come out of this workflow. The first `desktop-v*` tag to produce one is the one that measures it.
