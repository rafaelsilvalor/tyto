...
[`docs/render-contract.md`](./render-contract.md), with
[`render-result.schema.json`](./render-result.schema.json) beside it. Both are generated by
`pnpm docs:gen` from the code, so neither can drift from what Tyto actually writes. What
follows here is the same boundary described for somebody working _on_ Tyto.
