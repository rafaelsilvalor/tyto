...
- ca8f122: Say in `face.ts` that fontkit's purity is a property of the export condition (TYTO-71).
...
- 2cea94a: Adjacent runs that would draw the same thing are one run (TYTO-72).
...
- 9b44b9d: `min` on a non-repeatable rich-text slot is enforced (TYTO-67).
...
- 4519c36: A failed read says which kind of file failed (TYTO-73).
...
- a40f4d9: Inline markup in a frontmatter scalar is now a warning (E3.5).
